...
export class HoosatTxBuilder {
   ...
   *
   * By default, transactions use the native subnetwork (0x00...00) where payload is disabled.
   * Alternative subnetwork IDs may allow payload before the hardforк.
   *
   ...
   * builder.setSubnetworkId('0200000000000000000000000000000000000000');
   *
   * @example
  ...
  setSubnetworkId(subnetworkId: string): this {
    ...
    if (!/^[0-9a-f]{40}$/.test(cleanId)) {
      throw new Error(
        ...
        `Expected 40 hex characters (20 bytes), e.g., "0200000000000000000000000000000000000000"`
      );
    }
   ...
   * builder
   *   .setSubnetworkId('0200000000000000000000000000000000000000')
   ...
   * @param payloadSize - Size of payload in bytes (default: 0)
   * @returns Estimated minimum fee as string
   * @example
