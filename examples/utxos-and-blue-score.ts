...
async function demonstrateNewMethods() {
  ...
  try {
    ...
    if (utxosResult.ok && utxosResult.result) {
      ...
      Object.entries(utxosByAddress).forEach(([address, utxos]) => {
        ...
        console.log(`   Баланс: ${HoosatUtils.sompiToAmount(totalAmount.toString())} HTN`);
  ...
  try {
    ...
    if (utxosResult.ok && utxosResult.result) {
      ...
      console.log(`💰 Доступный баланс: ${HoosatUtils.sompiToAmount(totalBalance.toString())} HTN`);
      ...
      const targetAmount = HoosatUtils.amountToSompi('1.0'); // 1 HTN в sompi
      ...
      if (selectedUtxos.length > 0) {
        ...
        console.log(`   Общая сумма: ${HoosatUtils.sompiToAmount(selectedAmount.toString())} HTN`);
        console.log(`   Сдача: ${HoosatUtils.sompiToAmount((selectedAmount - BigInt(targetAmount)).toString())} HTN`);
      } else {
