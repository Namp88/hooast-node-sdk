...
async function testAll() {
  ...
  await test('Test 3: Get Balance', async () => {
    ...
    console.log(`   Balance: ${node.formatAmount(balance.balance)} HTN`);
  ...
  await test('Test 8: Get UTXOs', async () => {
    ...
    if (utxos.length > 0) {
      console.log(`   First UTXO amount: ${node.formatAmount(utxos[0].utxoEntry.amount)} HTN`);
  ...
  await test('Test 16: Format/Parse Amounts', async () => {
    ...
    console.log(`   ${sompi} sompi = ${hoo} HTN`);
    console.log(`   ${hoo} HTN = ${backToSompi} sompi`);
